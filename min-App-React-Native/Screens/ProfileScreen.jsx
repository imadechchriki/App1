...
function UserScreen() {
  ...
  const getAllData = async () => {
    try {
      const response = await axios.get('http://100.73.108.45:5001/get-all-user');
    ...
    <View style={styles.container}>
      <View style={styles.searchBar}>
        ...
        <Text style={styles.recordsText}>
          {searchQuery ? `${filteredUsers.length} records found` : `Total Records ${allUserData.length}`}
        </Text>
      </View>
      <FlatList
        data={filteredUsers.length > 0 || searchQuery ? filteredUsers : allUserData}
